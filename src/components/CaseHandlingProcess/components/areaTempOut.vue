<template>
  <el-scrollbar
    v-loading="loadingInstance"
    wrap-class="case-handling-process-scroll"
    ref="scrollbar"
  >
    <div id="areaTempOut" class="areaTempOut">
      <div class="areaTempOutTableBox">
        <div class="tableContent">
          <div class="tableContentItem temporaryLeaveTable">
            <div class="tableItemTitle"><span></span>临时出入区</div>
            <div
              class="tableBox"
              v-for="(item, index) in temporaryLeaveInfo"
              v-show="temporaryLeaveInfo && temporaryLeaveInfo.length"
              :key="index"
            >
              <div class="tableItemContent tableSingItemContent">
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">临时离开时间</div>
                    <div
                      class="tableItemCon"
                      :title="
                        item.leaveTime
                          ? $moment(item.leaveTime).format(
                              'YYYY-MM-DD HH:mm:ss'
                            )
                          : ''
                      "
                    >
                      {{
                        item.leaveTime
                          ? $moment(item.leaveTime).format(
                              'YYYY-MM-DD HH:mm:ss'
                            )
                          : ''
                      }}
                    </div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">临时离开原因</div>
                    <div
                      class="tableItemCon"
                      :title="
                        item.leaveReason == '0'
                          ? item.otherReason
                          : temporaryLeaveReasonMap[item.leaveReason]
                      "
                    >
                      {{
                        item.leaveReason == '0'
                          ? item.otherReason
                          : temporaryLeaveReasonMap[item.leaveReason]
                      }}
                    </div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">登记人</div>
                    <div
                      class="tableItemCon"
                      :title="item.leaveRegistrantName || ''"
                    >
                      {{ item.leaveRegistrantName || '' }}
                    </div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">返还时间</div>
                    <div
                      class="tableItemCon"
                      :title="
                        item.backTime
                          ? $moment(item.backTime).format('YYYY-MM-DD HH:mm:ss')
                          : ''
                      "
                    >
                      {{
                        item.backTime
                          ? $moment(item.backTime).format('YYYY-MM-DD HH:mm:ss')
                          : ''
                      }}
                    </div>
                  </li>
                </el-col>
                <el-col :span="24">
                  <li class="tableItem tableSignItem">
                    <div class="tableItemLabel">管理员签字</div>
                    <div class="tableItemCon">
                      <div
                        class="signContent"
                        v-lazy-container="{ selector: 'img' }"
                      >
                        <img
                          :data-src="item.managerSign"
                          :data-error="signImg"
                          :data-loading="signImg"
                          alt=""
                        />
                      </div>
                      <!-- {{ temporaryLeaveInfo.personSign }} -->
                    </div>
                  </li>
                </el-col>
              </div>
            </div>
            <div
              class="tableBox"
              v-show="!temporaryLeaveInfo || !temporaryLeaveInfo.length"
            >
              <div class="tableItemContent tableSingItemContent">
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">临时离开时间</div>
                    <div class="tableItemCon"></div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">临时离开原因</div>
                    <div class="tableItemCon"></div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">登记人</div>
                    <div class="tableItemCon"></div>
                  </li>
                </el-col>
                <el-col :span="12">
                  <li class="tableItem">
                    <div class="tableItemLabel">返还时间</div>
                    <div class="tableItemCon"></div>
                  </li>
                </el-col>
                <el-col :span="24">
                  <li class="tableItem tableSignItem">
                    <div class="tableItemLabel">管理员签字</div>
                    <div class="tableItemCon">
                      <div
                        class="signContent"
                        v-lazy-container="{ selector: 'img' }"
                      >
                        <img
                          :data-src="signImg"
                          :data-error="signImg"
                          :data-loading="signImg"
                          alt=""
                        />
                      </div>
                    </div>
                  </li>
                </el-col>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </el-scrollbar>
</template>

<script>
import server from '../../../server/ledgerManage'
export default {
  name: 'areaTempOut',
  props: {
    currentRegisterId: {
      type: String,
      default: '',
    },
    currentIndexCode: {
      type: String,
      default: '',
    },
    processCode: {
      type: String,
      default: '',
    },
    inqDictList: {
      type: Object,
      default: () => {
        return {}
      },
    },
  },
  watch: {
    inqDictList: {
      handler: function (newV, oldV) {
        if (this.inqDictList) {
          this.temporaryLeaveReasonMap = this.computedDic(
            this.inqDictList['inqfd.temporaryLeaveReason']
          )
        }
      },
      immediate: true,
      deep: true,
    },
    processCode(indexCode) {
      console.log(indexCode)
      this.findTemporaryLeaveInfo()

    },
  },
  data() {
    return {
      temporaryLeaveInfo: [], // 临时离区
      temporaryLeaveReasonMap: {},
      loadingInstance: false,
      signImg:
        'data:image/png;base64,',
    }
  },
  mixins: [],
  methods: {
    // 临时出区信息
    async findTemporaryLeaveInfo() {
      this.loadingInstance = true
      try {
        let params = {
          indexCode: this.processCode
        }
        let res = await server.findTemporaryLeaveInfo(params)
        this.loadingInstance = false
        if (res.data.code === '0') {
          let data = res.data.data
          this.temporaryLeaveInfo = [res.data.data]
        } else {
          this.$messagebox(res)
        }
      } catch {
        this.loadingInstance = false
      }
    },
    computedDic(list) {
      let dicMap = {}
      if (list && list.length) {
        for (let i = 0; i < list.length; i++) {
          dicMap[list[i].key] = list[i].name
        }
      }
      return dicMap
    },
  },
  created() {
    if (this.inqDictList) {
      this.temporaryLeaveReasonMap = this.computedDic(
        this.inqDictList['inqfd.temporaryLeaveReason']
      )
      this.findTemporaryLeaveInfo()
    }
  },
  components: {
  },
}
</script>

<style lang="less" rel="stylesheet/less">
.areaTempOut {
  width: 100%;
  height: 100%;
  .areaTempOutTableBox {
    width: 100%;
    // padding: 40px;
    // margin: 40px auto;
    // box-shadow: 0 4px 12px 0 rgba(0, 0, 0, 0.2);
    background: #fff;
    height: auto;
    box-sizing: border-box
  }
  .tableContent {
    width: 100%;
    .tableContentItem {
      height: 220px;
      &.temporaryLeaveTable {
        padding-bottom: 30px;
        height: auto;
        .tableBox {
          height: 180px;
        }
      }
      .tableItemTitle {
        height: 40px;
        line-height: 40px;
        box-sizing: border-box;
        font-size: 14px;
        > span {
          display: inline-block;
          background: #2080f7;
          width: 4px;
          height: 16px;
          position: relative;
          top: 4px;
          margin-right: 8px;
        }
      }
      .tableBox {
        height: 150px;
        margin-top: 16px;
        box-sizing: border-box;
        .tableItemContent {
          height: 77px;
          border-top: 1px solid #ddd;
          border-bottom: none;
          border-right: none;
          .el-col {
            height: 38px;
            padding-left: 0px!important;
            padding-right: 0px!important;
            .tableItem {
              width: 100%;
              height: 100%;
              border-bottom: 1px solid #ddd;
              border-left: 1px solid #ddd;
              list-style: none;
              .tableItemLabel {
                width: 120px;
                position: relative;
                text-indent: 14px;
                font-family: PingFangSC-Semibold;
                font-size: 14px;
                color: #999999;
              }
              .tableItemLabel::before {
                content: '';
                display: block;
                position: absolute;
                width: 100%;
                height: 100%;
                opacity: 0.3;
                top: 0;
                left: 0;
                z-index: 1;
                background: #f2f2f2;
              }
              .tableItemCon {
                width: ~'calc(100% - 120px)';
                text-align: left;
                text-indent: 14px;
                font-family: PingFangSC-Regular;
                font-size: 14px;
                color: #4d4d4d;
                overflow: hidden;
                text-overflow: ellipsis;
                white-space: nowrap;
              }
              > div {
                height: 38px;
                float: left;
                line-height: 38px;
                text-indent: 14px;
                border-right: 1px solid #ddd;
              }
            }
          }
          &.tableSingItemContent {
            height: 38px;
            .el-col:nth-last-of-type(1) {
              height: 80px;
              .tableSignItem {
                height: 80px;
                border-left: 1px solid #ddd;
                .tableItemLabel {
                  height: 80px;
                  line-height: 80px;
                }
                .tableItemCon {
                  height: 80px;
                  overflow: hidden;
                  text-overflow: ellipsis;
                  white-space: nowrap;
                  .signContent {
                    width: 120px;
                    height: 80px;
                    padding: 4px;
                    box-sizing: border-box;
                    img {
                      width: 100%;
                      height: 100%;
                    }
                  }
                }
              }
            }
          }
        }
      }
    }
  }
}
</style>
