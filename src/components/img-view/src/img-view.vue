        ...
        if ($imgView) {
          ...
          const getContainSize = () => {
            ...
            if (whRatio <= elWhRatio) {
              size = {
                width: elHeight * whRatio,
                height: elHeight
              };
            } else {
              size = {
                width: elWidth,
                height: elWidth / whRatio
              };
            }
            return size;
          };
    ...
    $zoomOut(e) {
      ...
      if (!this.lock) {
        if(e){
          this.zoom({
            e,
            // currentScale: this.eventData.scale - this.radix,
          ...
          this.zoom({
            e,
            // currentScale: this.eventData.scale - this.step,
            currentScale: this.eventData.scale / (1 + this.radix),
            type: 'zoomOut'
          });
        }
